...
public class WithinFilter extends FilterBase {
    ...
    public WithinFilter(final Geometry query, byte[] family, byte[] lonCol, byte[] latCol) {
        this.query = query;
        this.family = family;
        this.lon_col = lonCol;
        this.lat_col = latCol;
    ...
    public ReturnCode filterCell(Cell cell) throws IOException {
        ...
        if (cell_lon.isPresent() && cell_lat.isPresent()) {
            if (Double.isNaN(cell_lon.get()) || Double.isNaN(cell_lat.get())) {
                LOG.debug("Either lon or lat are NaN");
